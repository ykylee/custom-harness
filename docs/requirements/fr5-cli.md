...
- 상태: approved (v1.1, 2026-09-01 — FR-5.4 범위 제한 해제(FR-9.6 승인, M7 7.5.1). v1: 2026-08-25 사용자 승인)
- 최종 수정일: 2026-09-01
...
## FR-5.4 범위 제한 (2026-09-01 해제)

- ~~세션 생성·프롬프트 실행 등 에이전트 조작 명령은 1차 범위에서 제외한다 (UI 가 유일한 조작면). 필요성이 확인되면 M4 에서 재검토.~~
- **해제됨** — [FR-9.6](./fr9-orchestration.md) 승인으로 조작 명령이 CLI 표면에 편입됐다(M7 7.5). 자동화가 1급 경로라면 조작이 UI 안에만 있어서는 안 된다. 구현 범위·출력 규약은 FR-9.6 과 [m7 로드맵 WP 7.5](../roadmap/m7-orchestration.md) 를 따른다.
