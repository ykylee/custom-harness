...
- 상태: approved (v1.5, 2026-09-01 — §4.1 에 팬아웃 상한·비용 합산 실측 추가(7.3.2). v1.4: §4.1 에 위임 루프 실측 추가(7.3.1, 3하네스 PASS). v1.3: §4.1 write 툴 왕복 실측 추가(7.2.4, 3하네스 PASS). v1.2: §3.3 grok 권한 모드를 WBS 7.2.0b 로 확정(원인이 §3.1 누수였음을 실측). v1.1: §3.1 격리 누수 봉쇄. v1.0: 7.2.1 실측)
- 최종 수정일: 2026-09-01
...
자식 세션의 누적 사용량도 부모 쪽에서 조회된다.

### 팬아웃 상한·비용 합산 (2026-09-01, 7.3.2)

같은 프로브가 위임 루프 뒤에 **자식이 1개 살아 있는 상태에서 하나 더** 만들려 하고, 이어서 `session_usage` 를 부른다.

| 하네스 | 둘째 `session_new` | 자손 합(totalTokens) | 활성 자식 수 |
|---|---|---|---|
| omp 17.3.8 | **차단** (팬아웃 상한) | 208 | 1 |
| grok 1.0.13 | **차단** | 520 | 1 |
| pi 0.84.1 | **차단** | 208 | 1 |

차단 시 **승인 요청이 새로 뜨지 않는다** — 상한 판정이 승인보다 앞서므로 어차피 못 할 일로 사용자를 깨우지 않는다. 자손 합은 부모 자신의 사용량과 자식의 것을 더한 값이고, `session_usage` 가 둘을 나눠 준다.
