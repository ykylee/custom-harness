...
- 호환성 검수: `npm run audit:compat` (COMPAT 태그 형식·만료일)
...
- CI: GitHub-hosted `quality`는 typecheck·test·lint·format·COMPAT를, 사내 self-hosted macOS arm64 `release-nfr`는 번들 조립 후 NFR smoke·clean-room을 수행한다. `release-nfr`은 branch protection의 필수 검사로 지정한다.
