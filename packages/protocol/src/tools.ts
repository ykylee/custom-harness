...
// 파라미터는 와이어 스키마의 `looseObject` 관례를 따르지 않고 `z.object` 를 쓴다.
...
export const TOOL_CATALOG = [
  ...
  },
  {
    ...
    params: z.object({ sessionId, prompt: z.string().describe('보낼 프롬프트') }),
    effect: 'write',
    approval: true,
  },
  {
  ...
  },
  {
    ...
    params: z.object({ workspaceId: z.string(), cwd: z.string().optional() }),
    effect: 'write',
    approval: true,
  },
  {
    ...
    params: z.object({ terminalId: z.string(), data: z.string().describe('보낼 입력') }),
    effect: 'write',
    approval: true,
  },
  {
