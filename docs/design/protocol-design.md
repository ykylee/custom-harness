...
- **RPC**: `domain.verb.request` / `.response`, `requestId` 상관. 도메인: `session.*`(create/resume/list/close/prompt/interrupt/wait/result/usage/permission.respond/model.set/timeline/attention.ack — `wait`·`result` 는 M7 7.3.1 위임용, `usage` 는 7.3.2 비용 합산), `config.*`(key.set/key.test/get/set), `harness.*`(list/probe), `tool.*`(invoke — M7 7.2.4 역방향 툴), `system.*`(version/shutdown/about/license.read — `about`·`license.read` 는 M3 3.3.2 라이선스 고지 열람, FR-4.5). M5·M6 이 더한 `project.*`·`workspace.*`·`terminal.*`·`file.*`·`diff.*` 는 각 설계서가 소유한다.
