...
- 상태: approved (v1.1, 2026-08-31 — FR-2.5 에 원격 MCP 등록 검사 + 하네스 홈 격리 선행 조건 추가(M7 7.2.0a). v1: 2026-08-25 사용자 승인)
- 최종 수정일: 2026-08-31
...
- **검사 대상에 원격 MCP 서버 등록을 포함한다** (M7 7.2.0a, 2026-08-31): omp `mcp.json` 의 `mcpServers.*.url`, grok `config.toml` 의 `[mcp_servers.*].url`. MCP 서버는 LLM 엔드포인트가 아니지만 하네스가 게이트웨이 밖으로 직접 나가는 통로이므로 같은 경계에 둔다. stdio 등록은 목적지가 없어 위반이 아니다.
- **선행 조건 — 하네스 홈 격리**: 설정 홈 격리(`PI_CODING_AGENT_DIR`/`GROK_HOME`)만으로는 하네스가 사용자 실제 `$HOME` 의 외부 도구 MCP 설정을 읽는 것을 막지 못한다(7.2.1 실측). 데몬은 spawn 시 `HOME`·`USERPROFILE`·XDG 4종을 하네스별 빈 홈으로 덮어 그 유입 자체를 끊는다 — 그래야 위 검사가 "우리가 등록한 것만" 보면 되는 상태가 된다. 상세는 [하네스 MCP 지원 실측 §3.1](../reference/harness-mcp-support.md).
